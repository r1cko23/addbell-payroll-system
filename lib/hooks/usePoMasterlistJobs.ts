...
export const PO_MASTERLIST_PAGE_SIZES = [20, 50, 100] as const;
export const PO_MASTERLIST_PAGE_SIZE = 50;
/** Default backtrack window for Projects year filter options. */
...
async function fetchPoMasterlistJobs(
  ...
  return {
    ...
    syncErrorCount: json.syncErrorCount ?? 0,
    canSyncSheet: Boolean(json.canSyncSheet),
  };
}
...
export function usePoMasterlistJobs(
  ...
  return {
    ...
    syncErrorCount: query.data?.syncErrorCount ?? 0,
    canSyncSheet: query.data?.canSyncSheet ?? false,
...
export async function pullPoMasterlistSheet(): Promise<{
  inserted: number;
  updated: number;
  ...
  const res = await fetch("/api/po-masterlist/sheet-pull", {
    method: "POST",
    credentials: "include",
  });
  const json = (await res.json()) as {
  ...
  return {
    ...
    skippedHeader: json.skippedHeader ?? 0,
    skippedAppNewer: json.skippedAppNewer ?? 0,
    skippedInSync: json.skippedInSync ?? 0,
    errors: json.errors ?? 0,
    rowsRead: json.rowsRead ?? 0,
  };
}

export const PO_MASTERLIST_AUTO_PULL_INTERVAL_MS = 60_000;
export const PO_MASTERLIST_LAST_PULL_STORAGE_KEY =
  "po-masterlist-last-sheet-pull-ms";

export function shouldAutoPullPoMasterlistSheet(args: {
  canSyncSheet: boolean;
  lastPullAtMs: number | null;
  nowMs: number;
}): boolean {
  if (!args.canSyncSheet) return false;
  if (args.lastPullAtMs == null) return true;
  return args.nowMs - args.lastPullAtMs >= PO_MASTERLIST_AUTO_PULL_INTERVAL_MS;
}

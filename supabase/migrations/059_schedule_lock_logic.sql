-- =====================================================
-- Update replace_week_schedule: allow edits until Tuesday of the same week; AM/Admin always can
...
BEGIN
  ...
  -- If already exists and caller not AM/Admin, only allow until Tuesday (week_start + 1)
  IF v_existing > 0 AND (v_role IS NULL OR v_role NOT IN ('account_manager','admin')) THEN
    IF CURRENT_DATE > (v_week_start + INTERVAL '1 day') THEN
      RAISE EXCEPTION 'Week already submitted; edits allowed only until Tuesday';
    END IF;
  END IF;

...
async function getEmployeeUUID(employeeId: string): Promise<string | null> {
...
async function getOfficeLocation(): Promise<{ lat: number; lng: number } | null> {
  const { data, error } = await supabase
...
function parseDateTime(dateTimeStr: string): Date {
  ...
  for (const format of formats) {
    try {
      return parse(dateTimeStr, format, new Date());
    } catch (e) {
      continue;
    }
  }
...
async function createTimeClockEntry(options: Options): Promise<void> {

...
/** One values.batchUpdate data[] entry per masterlist row. */
export function buildPoMasterlistSheetBatchValueRanges(
  rows: Array<{ sheetTab: string; sheetRow: number; values: string[] }>
): Array<{
  range: string;
  majorDimension: "ROWS";
  values: string[][];
  ...
  return rows.map((row) => ({
    range: quotePoMasterlistSheetRowRange(row.sheetTab, row.sheetRow),
    majorDimension: "ROWS",
    values: [row.values],
  }));
}
...
export function poMasterlistSheetRowsEqual(
  left: PoMasterlistSheetRowSource,
  right: PoMasterlistSheetRowSource
): boolean {
  const a = buildPoMasterlistSheetRowValues(left);
  const b = buildPoMasterlistSheetRowValues(right);
  return a.length === b.length && a.every((cell, index) => cell === b[index]);
}

export function poMasterlistSheetRowFingerprint(
  source: PoMasterlistSheetRowSource
): string {
  return buildPoMasterlistSheetRowValues(source).join("\u0001");
}

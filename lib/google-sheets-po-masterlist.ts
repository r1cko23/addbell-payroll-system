...
export type PoMasterlistSheetRowWrite = {
  sheetTab: string;
  sheetRow: number;
  values: string[];
};

function assertPoMasterlistSheetRowWrite(params: PoMasterlistSheetRowWrite): {
  sheetTab: string;
  sheetRow: number;
  values: string[];
} {
  ...
  return { sheetTab, sheetRow: params.sheetRow, values: params.values };
}

/**
 * Push one or more full masterlist rows (A:N) in a single Sheets write request.
 * Caller must already have committed the jobs in Postgres.
 */
export async function updatePoMasterlistSheetRows(
  rows: PoMasterlistSheetRowWrite[]
): Promise<void> {
  ...
  const normalized = rows.map(assertPoMasterlistSheetRowWrite);
  ...
  await sheets.spreadsheets.values.batchUpdate({
    ...
    requestBody: {
      ...
      data: buildPoMasterlistSheetBatchValueRanges(normalized),
    },
  });
}
 ...
 */
export async function updatePoMasterlistSheetRow(
  params: PoMasterlistSheetRowWrite
): Promise<void> {
  await updatePoMasterlistSheetRows([params]);
}
